xpcall(function()
	...
	local ShouldUseNormalNoclip = false
	...
	local _, esplib = pcall(
		loadstring(game:HttpGet("https://raw.githubusercontent.com/MimiTest2/projectrainfree/main/esplib.lua", true))
	)
	...
	pcall(
		loadstring(game:HttpGet("https://raw.githubusercontent.com/MimiTest2/projectrainfree/main/irisinstprotect.lua"))
	)
				...
				if input.UserInputType == Enum.UserInputType.MouseButton1 then
					for i, v in pairs(plr.PlayerGui.LeaderboardGui.MainFrame.ScrollingFrame:GetChildren()) do
						...
						if Player.TextTransparency > 0 then
							while v.Player.TextTransparency > 0 do
								workspace.CurrentCamera.CameraSubject = Players[v.Player.Text].Character
								task.wait()
							end
						end
					end
	...
	if pgui:FindFirstChild("LoadingGui") then
		...
		repeat
			wait()
		until not pgui:FindFirstChild("LoadingGui")
	end
	...
	function doRegion(part)
		...
		for i, v in pairs(parts) do
			if not partids[v] then
				partids[v] = { Instance = v, Transparency = v.Transparency, CanCollide = v.CanCollide }
			end
			v.CanCollide = false
			v.Transparency = 0.75
		end
	...
	local CON_FUNCTIONS = {
		...
		Flight = function()
			xpcall(function()
				local hrp, hum =
					plr and plr.Character and getRoot(),
					plr and plr.Character and plr.Character:FindFirstChildOfClass("Humanoid")
		...
		["Speed"] = function()
			xpcall(function()
				local hrp, hum =
					plr.Character:FindFirstChild("HumanoidRootPart"), plr.Character:FindFirstChildOfClass("Humanoid")
				if not hrp or not hum then
					return
				end
		...
		Noclip = function()
			...
			xpcall(function()
				...
				local raycastResult = workspace:Raycast(
					...
					(
						(plr.Character.Torso.Position - plr.Character.HumanoidRootPart.Position)
						+ plr.Character.Torso.CFrame.LookVector
					) + plr.Character.Humanoid.MoveDirection.Unit * 5,
					raycastParams
				)
				if raycastResult and raycastResult.Instance.CanCollide then
					if not partids[raycastResult.Instance] then
						partids[raycastResult.Instance] = {
							Instance = raycastResult.Instance,
							Transparency = raycastResult.Instance.Transparency,
							CanCollide = raycastResult.Instance.CanCollide,
						}
					end
		...
		LPH_JIT(function()
			pcall(function()
				...
				call = hookmetamethod(
					game,
					"__namecall",
					LPH_NO_UPVALUES(function(Self, ...)
						local args = { ... }
						if not checkcaller() then
							local getname = getnamecallmethod()
							if getgenv().Nofall then
								if
									tostring(getname) == "FireServer"
									and getcallingscript().Name == "WorldClient"
									and type(args[1]) == "number"
									and type(args[2]) == "boolean"
								then
				...
				oldnewindex = hookmetamethod(
					game,
					"__newindex",
					LPH_NO_UPVALUES(function(self, prop, val)
						if not checkcaller() then
							...
							if self:IsA("Atmosphere") and self.Parent.Name == "Lighting" and getgenv().NoFog then
								if prop == "Density" then
									return oldnewindex(self, prop, 0.002)
								end
								return
							end
	...
	addToggle("Normal Noclip", "NormalNoclip", false, "Movement", function(e)
		ShouldUseNormalNoclip = e
	end)

	addToggle("Noclip", "Noclip", false, "Movement", function(e)
		if ShouldUseNormalNoclip and not e then
			-- ported from https://github.com/MimiTest2/projectrainfree/blob/main/rain%20v3.lua
			pcall(function()
				for i, v in pairs(lplr.Character:GetChildren()) do
	...
	end, true, Enum.KeyCode.Z, "Noclip")
		...
		pcall(function()
			while getgenv().INTFARMENABLED do
				...
				xpcall(function()
					repeat
						task.wait()
					until pgui:FindFirstChild("ChoicePrompt")
					repeat
						task.wait()
					until pgui:FindFirstChild("ChoicePrompt"):FindFirstChild("ChoiceFrame")
	...
	tabs["Misc"]:CreateButton("Mod Office", function()
		...
		xpcall(function()
			local char, hrp =
				plr and plr.Character, plr and plr.Character and plr.Character:FindFirstChild("HumanoidRootPart")
			if not plr or not char or not hrp then
				return
			end
		...
		function esp(drop)
			...
			renderstepped = runservice.Heartbeat:Connect(LPH_JIT(function()
				local suc, err = pcall(function()
					if
						drop
						and workspace.Live[drop.Name]
						and drop:FindFirstChild("Humanoid")
						and drop:FindFirstChild("HumanoidRootPart")
					then
		...
		xpcall(function()
			...
			if data[anim] then
				...
				if library.flags["APA"] then
					info.Wait = info.Wait
						- game:GetService("Stats"):WaitForChild("PerformanceStats"):WaitForChild("Ping"):GetValue()
				end
				...
				if
					not library.flags["APD"]
					or (character:GetPivot().Position - plr.Character:GetPivot().Position).Magnitude > 50
				then
					return
				end
	...
	function charHandler(character) --g
		xpcall(function()
			...
			repeat
				task.wait(5)
			until character.Parent ~= workspace:FindFirstChild("Live")
	...
	addToggle("AP", "", false, "Combat", function(val) end)
	addSlider("Combat", "PA", -25, 0, 100, "PA")
	addToggle("APD", "", true, "Combat", function(val) end)
	addToggle("APA", "", true, "Combat", function(val) end)

	addToggle("OR", "", false, "Combat", function(val) end)
	LoadingText:Destroy()
